...
# Bytechcoin Multi-installer
# a one line clone-and-compile for bytechcoin:
#
#     ` $ curl -sL "https://raw.githubusercontent.com/bytechcoin/bytechcoin/master/scripts/multi_installer.sh" | bash
#
...
_set_wd() {
    if [ -d "$PWD/.git" ] && [ -f "$PWD/Makefile" ]; then
        ...
        if [ -d "$PWD"/bytechcoin ]; then
            read -r -p "${1:-bytechcoin directory already exists. Overwrite? [y/N]} " response
            case "$response" in
                [yY][eE][sS|[yY])
                    _colorize red "Overwriting old bytechcoin directory" && echo
                    rm -rf "$PWD"/bytechcoin
                    ;;
                *)
                    _fail "bytechcoin directory already exists. Aborting..."
                    ;;
            esac
        fi
        mkdir bytechcoin
        git clone -b master -q https://github.com/bytechcoin/bytechcoin bytechcoin   >>build.log 2>&1 || _fail "Unable to clone git repository. Please see build.log for more information"
        cd bytechcoin
    fi
}

_build_bytechcoin() {
    _note "Building bytechcoin from source (this might take a while)..."
    if [ -d build ]; then
...
_configure_linux() {
    if [ "$(awk -F= '/^NAME/{print $2}' /etc/os-release)" = "\"Ubuntu\"" ]; then
        ...
        _fail "Your OS version isn't supported by this installer. Please consider adding support for your OS to the project ('https://github.com/bytechcoin')"
    fi
}
...
_configure_os() {
    ...
    case "${_unameOut}" in
        ...
        Darwin*)
            ...
            ;;
        *)
            _fail "This installer only runs on OSX 10.10+ and Ubuntu 16.04+. Please consider adding support for your OS to the project ('https://github.com/bytechcoin')"
            ;;
    esac
    _note "Operating system configuration completed. You're halfway there!"
}

_note "Bytechcoin Multi_Installer v1.0 (pepperoni)"
...
_note "Look in 'bytechcoin/build/src/' for the executible binaries. See 'https://github.com/bytechcoin/bytechcoin' for more project support. Cowabunga!"
