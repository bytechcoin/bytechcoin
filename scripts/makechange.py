...
Open the wallets with bytech-serivce like so:

./bytech-service -w walletA.wallet -p yourpass --rpc-password test --bind-port 8070
./bytech-service -w walletB.wallet -p yourpass --rpc-password test --bind-port 8071
...
moveDecimal = 100  # BYTC has 2 decimals so 100 is the divide/multiply factor
minAmount = 100 * moveDecimal  # min number for amount to xfer
maxAmount = 5000 * moveDecimal  # max number for amount to xfer
anonymity = 3
fee = 10 # atomic units, BYTC would be 0.10 as the tx network fee
